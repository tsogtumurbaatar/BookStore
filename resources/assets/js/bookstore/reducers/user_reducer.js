...
export const user = (state = INITIAL_STATE, action)=> {
  ...
  switch(action.type) {
    ...
    return Object.assign({}, state, {user: null, status:'validate_email', error:null, loading: true});
    case VALIDATE_EMAIL_SUCCESS:
    return Object.assign({}, state, {user: action.payload.data.user, status:'authenticated', error:null, loading: false}); //<-- authenticated & email verified
    case VALIDATE_EMAIL_FAILURE:
    error = action.payload.data || {message: action.payload.message};//2nd one is network or server down errors       
    return Object.assign({}, state, {user:null, status:'validate_email', error:error, loading: false}); //<-- authenticated
    ...
    return Object.assign({}, state, {user: action.payload.data.user, status:'authenticated', error:null, loading: false}); //<-- authenticated
    ...
    return Object.assign({}, state, {user: null, status:'signin', error:null, loading: true}); 
      case SIGNIN_USER_START:// sign in user,  set loading = true and status = signin
    return Object.assign({}, state, {user: null, status:'signin', error:null, loading: true}); 
